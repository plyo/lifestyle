...
function getContact(container) {
    var contact = container.getElementsByTagName("finn:contact")[0];

    if (!contact) {
        return null;
    }

    var nameEl = firstChildOfType(contact, 'name');
    var emailEl = firstChildOfType(contact, 'email');
    var phoneEl = firstChildOfType(contact, 'finn:phone-number');
...
function parsePrices(children) {
    ...
    children.forEach(function(e) {
        var includes = childrenOfType(e, 'finn:field').filter(function (el) {
            return el.getAttribute('name') === 'includes';
        }).map(function (el) {
            return el.textContent;
        });

        ret[e.getAttribute('name')] = {
            value: parseInt(e.getAttribute('value')),
            includes: includes
        };
    });
    return ret;
...
function parseFinnFields(models, root) {
    ...
    fields.forEach(function(e) {
        var value = parseFinnField(models, e);
        if (value) ret[e.getAttribute('name')] = parseFinnField(models, e);
    });
    return ret;
}
...
function parseFinnSingleValue(model, ele) {
    ...
    var from = ele.getAttribute("from");
    ...
    if (type === "complex") {
        ...
        return {
            from: from && isFinite(from) ? castValue('number', from) : typeConvert(from, 'time') || null,
            to: to && isFinite(to) ? castValue('number', to) : typeConvert(to, 'time') || null
        };
    }
    else {
